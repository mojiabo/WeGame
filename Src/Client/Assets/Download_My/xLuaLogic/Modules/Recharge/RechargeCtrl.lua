...
function RechargeCtrl.GetServerRechargeProduct(rechargeProductId)
	...
	for i=1, #productRechargeTable, 1 do
		--print("productRechargeTable"..productRechargeTable[i].RechargeProductId);
	...
	local rechargeShopTable = DBModelMgr.GetDBModel(DBModelNames.RechargeShopDBModel).GetList();--本地数据表
		...
		for i=1, #rechargeShopTable, 1 do
			...
			while true do
				...
				if serverRechargeProduct==nil then
				break;
				end
				--print("Type"..rechargeShopTable[i].Id);
				--print("CanBuy"..serverRechargeProduct);
				...
				CS.LuaHelper.Instance:AutoLoadTexture(imgIcon.gameObject,string.format("Download/Source/UISource/Recharge/%s.assetbundle",rechargeShopTable[i].Icon), rechargeShopTable[i].Icon);
				--元宝
